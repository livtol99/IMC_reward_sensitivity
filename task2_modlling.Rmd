...
pacman::p_load("tidyverse", "ggpubr", "Hmisc", "corrplot", "rstatix", "psych", "nlme", "lme4", "lmerTest", "JWileymisc","multilevelTools", "texreg", "ordinal", "GGally")

#The ordinal package allows for creating regression models for ordinal data
...
#Fitting the "full" intended models
#Random intercept models
antmod <- clmm(AntPleasure ~ EEfRT_averaged_lowp + EEfRT_averaged_medp + EEfRT_averaged_highp + (1|ID) + (1|subj_day), data = ESM_merged, na.action = "na.omit", Hess = TRUE, link = 'logit') #logit link gives us the proportional odds mixed model

summary(antmod)

conmod <- clmm(ConPleasure ~ EEfRT_averaged_lowp + EEfRT_averaged_medp + EEfRT_averaged_highp + (1|ID) + (1|subj_day), data = ESM_merged, na.action = "na.omit", Hess = TRUE, link = 'logit')
...
# 2. One or more of the predictors are either continuous, categorical, or ordinal
...
#Assumption that the effect of each independent variable is the same for each change point in the dependent variable
#The odds of the change from each point on the Likert scale as an effect of the predictors should be proportional
#We can do the brant test, and we hope for it to be non-significant, since the null hypothesis is parallell lines

pacman::p_load("brant")
brant(antmod) #none of the p-values are significant, and this the assumption holds

ESM_merged$ID <- as.numeric(ESM_merged$ID, na.action(na.omit))
ESM_merged$ <- as.numeric(ESM_merged$ID, na.action(na.omit))
class(ESM_merged$ID)
```
